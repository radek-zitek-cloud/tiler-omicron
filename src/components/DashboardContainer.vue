...
<template>
  <div class="dashboard-container">
    ...
    <div class="dashboard-header">
      ...
      <!-- Status Bar -->
      <div class="dashboard-status mt-2">
        <small class="text-muted">
          {{ dashboardStore.tiles.length }} tiles • 
          {{ dashboardStore.gridColumns }} columns • 
          {{ dashboardStore.currentBreakpoint }} layout
        </small>
        
        <div v-if="operationMessage" class="ms-3">
...
function showConfirmation(
  ...
  if (confirmationModal.value) {
    const modal = new (window as unknown as { bootstrap: { Modal: new (element: HTMLElement) => { show(): void } } }).bootstrap.Modal(confirmationModal.value);
    modal.show();
  }
}
